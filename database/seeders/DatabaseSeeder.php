...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $pollOptionMap = $this->seedPostPollOptions($pollIds);
        $this->seedPostPollVotes($relationCount, $pollIds, $pollOptionMap, $userIds, $now);
    ...
    private function seedPostPollOptions(array $pollIds): array
    {
        if ($pollIds === []) {
            return [];
        }
    ...
    private function seedPostPollVotes(int $relationCount, array $pollIds, array $pollOptionMap, array $userIds, $now): void
    {
    ...
    private function seedReports(
        ...
        $rows = array_merge(
            $rows,
            $this->buildReportRows(Post::class, $postIds, $userIds, $relationCount, $reasons, $now)
        );
        $rows = array_merge(
            $rows,
            $this->buildReportRows(Message::class, $messageIds, $userIds, $relationCount, $reasons, $now)
        );
        $rows = array_merge(
            $rows,
            $this->buildReportRows(Space::class, $spaceIds, $userIds, $relationCount, $reasons, $now)
        );
        $rows = array_merge(
            $rows,
            $this->buildReportRows(Hashtag::class, $hashtagIds, $userIds, $relationCount, $reasons, $now)
        );
        $rows = array_merge(
            $rows,
            $this->buildReportRows(UserList::class, $userListIds, $userIds, $relationCount, $reasons, $now)
        );
        $rows = array_merge(
            $rows,
            $this->buildReportRows(User::class, $userIds, $userIds, $relationCount, $reasons, $now, true)
        );
